...
* (theimo1221) Add the COOLO CS-201Z (`ZigbeeCooloSoilSensor`, name your ioBroker device `00-Zigbee-<room>-CooloSoil-<index>`). Beyond soil and battery it adds the ambient temperature and humidity axes and counts towards the room aggregates for both, because the sensor sits in a pot in the room air; a sensor in a bed or a lawn would want the soil cluster alone. Its configuration states (`soil_calibration`, `soil_warning`, `temperature_calibration`, the two sampling intervals, `temperature_unit`) and the `dry` flag it derives from its own threshold are not mapped and stay reachable through `send_payload`
* (theimo1221) Record the daily precipitation in `iWeatherDaySummary` and the `WeatherDaySummary` table, filled from the day summary the backfill already fetches - no additional request, and existing installations get the column added once rather than the table recreated. The field is **optional in both directions**: an implementer of the aggregate keeps compiling, and - the part that matters - precipitation is read outside the completeness check that decides whether a day counts at all. Had it joined that check, every answer without the field would have discarded the whole day and the dachs start decision would have lost its history one gradually thinning window at a time. A refetch of the running day that arrives without precipitation keeps the figure an earlier one delivered instead of erasing it, and an absent value is stored as null rather than as a 0 that would claim the day was dry
* (theimo1221) Together these three make a later watering decision possible from recorded data - soil moisture over time against rainfall over time - without that decision existing yet. What is deliberately **not** done: the weather backfill still runs only where an energy manager drives it, so an installation without one records no weather history
...
* (theimo1221) Add Unifi Air Quality Sensor Support
...
* (theimo1221) Update pipeline to node 25
...
* (theimo1221) Motion sensor time since last motion

## 3.3.2 (2026-01-04)

* (theimo1221) Make shuttercallback accessible in json
...
* (theimo1221) Update iRoomBase interface

## 3.2.28 (2025-12-27)

* (theimo1221) Make room position accessible
...
* (theimo1221) Adjustments for temp correction
...
* (theimo1221) Increase Block duration to 15 minutes
