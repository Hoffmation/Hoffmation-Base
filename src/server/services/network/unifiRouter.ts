  ...
  public reconnectDeviceByMac(mac: string): Promise<boolean> {
    return new Promise(async (resolve, _reject) => {
      if (!this.loggedIn && !(await this.login())) {
        ServerLogService.writeLog(LogLevel.Warn, `Can't reconnect Device "${mac}" as we can't log in`);
        resolve(false);
      }
      const result = await this._api.reconnectClient(mac).catch((error: unknown) => {
        ServerLogService.writeLog(LogLevel.Warn, `Failed to reconnect "${mac}" due to: ${error}`);
        resolve(false);
        return;
      });
      ServerLogService.writeLog(LogLevel.Trace, `Reconnecting ${mac} resulted in ${result}`);
      resolve(true);
    });
  }
